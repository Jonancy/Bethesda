...
import { deleteNews } from "@/Services/newsArticles/newsArticles.service";

const handleDeleteNewsArticle = async (articleId: string) => {
  try {
    // Call the backend endpoint to delete the news article
    const res = await deleteNews(articleId);
    console.log(articleId);
    toast.success("The news article has been deleted successfully.");
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleDeleteClick = () => {
        console.log(article);
        handleDeleteNewsArticle(article.id);
        closeDialog();
      };
              ...
              <Dialog.Portal>
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-8 shadow-lg">
                  ...
                  <Dialog.Description className="mt-2 text-gray-500">
                    This will permanently delete the news article. This action
                    cannot be undone.
                  </Dialog.Description>
...
export function NewsDetailsTable() {
  const { data, loading, error } = GetAllArticlesAdmin();
